...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-blue-800 text-white flex-shrink-0 hidden md:block">
        <div class="h-full flex flex-col">
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                <a href="products.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'products' ? 'bg-blue-700' : 'hover:bg-blue-700' ?> transition-colors">
                    <i class="fas fa-box w-5"></i>
                    <span>Products</span>
                </a>
                <a href="orders.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'orders' ? 'bg-blue-700' : 'hover:bg-blue-700' ?> transition-colors">
                    <i class="fas fa-shopping-cart w-5"></i>
                    <span>Orders</span>
                </a>
                <a href="reports.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'reports' ? 'bg-blue-700' : 'hover:bg-blue-700' ?> transition-colors">
                    <i class="fas fa-chart-bar w-5"></i>
                    <span>Reports</span>
                </a>
                <?php if (getUserRole() === 'superadmin'): ?>
                <a href="users.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'users' ? 'bg-blue-700' : 'hover:bg-blue-700' ?> transition-colors">
                    <i class="fas fa-users w-5"></i>
                    <span>Manage Staff</span>
                </a>
                <?php endif; ?>
                <a href="notifications.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'notifications' ? 'bg-blue-700' : 'hover:bg-blue-700' ?> transition-colors relative">
            ...
            <div class="p-4 border-t border-blue-700">
                <div class="flex items-center space-x-3 mb-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-blue-200 truncate"><?= $_SESSION['role'] === 'superadmin' ? 'Admin' : ucfirst($_SESSION['role']) ?></p>
                    </div>
                </div>
    ...
    <aside x-show="sidebarOpen" x-cloak x-transition class="fixed inset-y-0 left-0 w-64 bg-blue-800 text-white z-50 md:hidden">
        <div class="h-full flex flex-col">
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                <a href="dashboard.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'dashboard' ? 'bg-blue-700' : 'hover:bg-blue-700' ?>">
                    <i class="fas fa-tachometer-alt w-5"></i>
                    <span>Dashboard</span>
                </a>
                <a href="products.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'products' ? 'bg-blue-700' : 'hover:bg-blue-700' ?>">
                    <i class="fas fa-box w-5"></i>
                    <span>Products</span>
                </a>
                <a href="orders.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'orders' ? 'bg-blue-700' : 'hover:bg-blue-700' ?>">
                    <i class="fas fa-shopping-cart w-5"></i>
                    <span>Orders</span>
                </a>
                <a href="reports.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'reports' ? 'bg-blue-700' : 'hover:bg-blue-700' ?>">
                    <i class="fas fa-chart-bar w-5"></i>
                    <span>Reports</span>
                </a>
                <?php if (getUserRole() === 'superadmin'): ?>
                <a href="users.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'users' ? 'bg-blue-700' : 'hover:bg-blue-700' ?>">
                    <i class="fas fa-users w-5"></i>
                    <span>Manage Staff</span>
                </a>
                <?php endif; ?>
                <a href="notifications.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg <?= $current_page === 'notifications' ? 'bg-blue-700' : 'hover:bg-blue-700' ?> relative">
                    <i class="fas fa-bell w-5"></i>
                    <span>Notifications</span>
                    <?php if ($unread_notifications > 0): ?>
                        <span class="absolute right-4 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
            ...
            <div class="p-4 border-t border-blue-700">
                <div class="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-200"><?= $_SESSION['role'] === 'superadmin' ? 'Admin' : ucfirst($_SESSION['role']) ?></p>
                    </div>
                </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="bg-white shadow-sm border-b border-gray-200">
            <div class="flex items-center justify-between px-4 py-3">
                ...
                <h1 class="text-2xl font-bold text-gray-800"><?= $page_title ?? 'Admin Panel' ?></h1>
