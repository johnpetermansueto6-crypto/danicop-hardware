...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($phone)) {
        ...
        $stmt = $conn->prepare("UPDATE drivers SET name = ?, phone = ?, email = ?, status = ?, vehicle_type = ?, license_number = ? WHERE id = ?");
        $stmt->bind_param("ssssssi", $name, $phone, $email, $status, $vehicle_type, $license_number, $id);
...
<form method="POST" action="content/driver_edit.php?id=<?= $id ?>" class="bg-white rounded-lg shadow-md p-6 max-w-2xl" onsubmit="handleFormSubmit(event, this, 'drivers'); return false;">
    <div class="mb-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <select name="status" required
                ...
                <option value="delivering" <?= $driver['status'] === 'delivering' ? 'selected' : '' ?>>Delivering</option>
        ...
        <div>
            ...
            <input type="text" name="vehicle_type" list="vehicleTypes"
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                   value="<?= htmlspecialchars($driver['vehicle_type'] ?? '') ?>">
