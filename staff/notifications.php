...
<head>
    ...
    <title>Notifications - Staff - Danicop Hardware</title>
...
<?php include '../includes/staff_sidebar.php'; ?>
    ...
    <div class="space-y-6">
        <div class="flex justify-between items-center mb-2">
            ...
            <a href="?read_all=1" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 text-sm">
                Mark All as Read
            </a>
        </div>
        ...
        <div class="space-y-4">
            <?php if ($notifications->num_rows > 0): ?>
                <?php while ($notif = $notifications->fetch_assoc()): ?>
                    <div class="bg-white rounded-lg shadow-md p-6 <?= $notif['is_read'] ? 'opacity-75' : 'border-l-4 border-green-600' ?>">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                <div class="flex items-center space-x-2 mb-2">
                                    ...
                                    <?php if (!$notif['is_read']): ?>
                                        <span class="bg-green-600 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">!</span>
                                    <?php endif; ?>
                                </div>
                            ...
                            <?php if (!$notif['is_read']): ?>
                                <a href="?read=<?= $notif['id'] ?>" class="text-green-600 hover:underline ml-4 text-sm">
                                    Mark as Read
                                </a>
                            <?php endif; ?>
                        </div>
